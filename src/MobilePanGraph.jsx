...
const MobilePanGraph = ({ data, setData }) => {
  ...
  const handlePointerDown = (e) => {
    // Only draw if it's the primary pointer or a single touch
    // To allow 2-finger panning, we ignore multi-touch
  ...
  return (
    <div style={{ width: '100%', overflowX: 'auto', touchAction: 'pan-x' }}>
      <p style={{ textAlign: 'center', color: '#94a3b8', fontSize: '13px', marginBottom: '8px' }}>
        Swipe with two fingers to pan. Draw with one finger.
      </p>
      <div style={{ width: '800px', minWidth: '200%' }}>
        <svg 
          ref={svgRef}
          className="svg-graph"
          ...
          style={{
            ...
            display: 'block',
            userSelect: 'none',
            touchAction: 'pan-x', // Allow horizontal panning native
            cursor: 'crosshair'
          }}
        >
