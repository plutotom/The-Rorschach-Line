...
const InteractiveGraph = ({ data, setData }) => {
  ...
  const updateDataAtPointer = useCallback(
    (clientX, clientY, isFirstTouch) => {
      ...
      // Guard: in iframes (e.g. Qualtrics) rect can be 0 before layout, causing NaN
      ...
      if (!Number.isFinite(age) || !Number.isFinite(happiness)) return;
      ...
      setData((prev) => {
        ...
        if (!isFirstTouch && lastCoord !== null) {
          ...
          for (let i = startAge + 1; i < endAge; i++) {
            const ratio = age === lastAge ? 0 : (i - lastAge) / (age - lastAge);
            const val = Math.round(
              lastHappiness + ratio * (happiness - lastHappiness),
            );
            newData[i] = Number.isFinite(val)
  ...
  const pathData = data
    .map((happiness, age) => {
      const x = age * 10;
      // Defensive: ensure numeric y (avoids NaN in iframe/embed when rect was 0 or state is sparse)
      const rawY = 500 - Number(happiness) * 5;
      const y = Number.isFinite(rawY) ? rawY : 500 - 50 * 5;
