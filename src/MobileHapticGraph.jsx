...
const MobileHapticGraph = ({ data, setData }) => {
  ...
  const updateDataAtPointer = useCallback((clientX, clientY, isFirstTouch) => {
    ...
    // If we cross a major grid line (0, 25, 50, 75, 100), we vibrate
    const snapLines = [0, 25, 50, 75, 100];
    let matchedLine = snapLines.find(line => Math.abs(happiness - line) <= 1);
    
    if (matchedLine !== undefined && matchedLine !== lastHapticValue.current) {
      if (typeof navigator !== 'undefined' && navigator.vibrate) {
        navigator.vibrate(15); // tiny tick
      }
      lastHapticValue.current = matchedLine;
    } else if (matchedLine === undefined && Math.abs(happiness - lastHapticValue.current) > 5) {
      // Clear out the haptic lock once we move far enough away
  ...
  return (
    <div>
      <p style={{ textAlign: 'center', color: '#94a3b8', fontSize: '13px', marginBottom: '8px' }}>
        If on a supported Android device, gives a haptic 'tick' when crossing major horizontal lines.
      </p>
      <svg 
        ref={svgRef}
        className="svg-graph"
...
export default MobileHapticGraph;
